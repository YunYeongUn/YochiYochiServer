    ...
    async function predict() {
        try {
            ...
            pose.keypoints.forEach((keypoint) => [
                coordArr.push(keypoint.position)
            ])
            canvas.width = video.width; //캔버스와 비디오의 크기를 일치시킴
            canvas.height = video.height;
            drawKeypoints(pose.keypoints, 0.6, context);
            await drawSkeleton(pose.keypoints, 0.6, context);
...
async function drawSkeleton(keypoints, minConfidence, ctx, scale = 1) {
    const adjacentKeyPoints = await posenet.getAdjacentKeyPoints(keypoints, minConfidence);

    adjacentKeyPoints.forEach((keypoints) => {
        drawSegment(toTuple(keypoints[1].position), toTuple(keypoints[0].position), color, scale, ctx);
    });
}
